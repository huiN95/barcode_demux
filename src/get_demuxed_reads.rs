...
fn get_subreads_from_demuxed_result(
    ...
    let select_position = |bc: &BarcodePair| {
        if keep_barcode {
            bc.outter_position
        } else {
            bc.inner_position
    ...
    match output_format {
        ...
        "fa" | "fasta" => {
            ...
            let fasta_rec = fasta::Record::with_attrs(&name, None, seq_slice);
            return Ok((RecordType::Fasta(fasta_rec), bc_opt));
        }
        ...
        "bam" => {
                ...
                Some(bc) => {
                    ...
                    let q_ok = if len_ok {
                        let q_slice = rec
                            ...
                            .ok_or_else(|| anyhow::anyhow!("missing quality"))?
                            .get(s..e)
                            .ok_or_else(|| anyhow::anyhow!("bad range qual: {}..{}", s, e))?;
            ...
            let qual_slice = rec
                ...
                .ok_or_else(|| anyhow::anyhow!("missing quality"))?
                .get(start..end)
                .ok_or_else(|| anyhow::anyhow!("bad range qual: {}..{}", start, end))?;

            let mut bam_rec = bam::Record::new();

            bam_rec.set(name.as_bytes(), None, seq_slice, qual_slice);
